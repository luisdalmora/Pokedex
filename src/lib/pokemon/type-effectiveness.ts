// Defesa: Quanto de dano o tipo na COLUNA recebe do tipo na LINHA (Ataque)
// Formato simplificado: { defensor: { atacante: multiplicador } }
...
export function calculateDefensiveTypeEffectiveness(types: string[]) {
  const effectiveness: Record<string, number> = {};

  // Initialize with 1x for all types
  ...
  for (const defenderType of types) {
    const typeDefenses = TYPE_CHART[defenderType.toLowerCase()];
    if (!typeDefenses) continue;
    
    for (const attackerType of TYPES) {
      if (typeDefenses[attackerType] !== undefined) {
        effectiveness[attackerType] *= typeDefenses[attackerType];
      }
    }
  }
