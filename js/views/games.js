    ...
    if (games && games.length > 0) {
      content.innerHTML = `
        <div class="games-grid">
          ${games.map(game => `
            <a href="#/games/${game.slug}" class="game-card hoverable-card" style="display: flex; flex-direction: column; background: var(--card-bg); border-radius: var(--radius); overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1); text-decoration: none; color: inherit; transition: transform 0.2s, box-shadow 0.2s;">
              <div style="padding: 15px; display: flex; align-items: center; justify-content: center; background-color: rgba(255,255,255,0.05); border-bottom: 2px dashed rgba(0,0,0,0.1);">
                <img src="./images/games/${game.slug}.png" alt="${game.name}" style="max-height: 200px; object-fit: contain; filter: drop-shadow(2px 4px 6px rgba(0,0,0,0.3));" onerror="this.style.opacity='0.1';" />
              </div>
              <div style="padding: 20px;">
                ...
                <span style="display: inline-block; margin-top: 15px; font-weight: bold; color: ${game.color}">Ver Detalhes &rarr;</span>
              </div>
            </a>
          `).join('')}
        </div>
